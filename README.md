...
## QC 
The raw dataset consisted of 156,572 cells across 25,129 features. Preliminary QC showed an imbalance in cell distribution across organs, with the OM and RM contributing the highest cell counts compared to LNG samples. QC metrics showed high sequencing depth (UMI counts > 3000) and high library complexity (log10 genes per UMI > 0.8). Mitochondrial ratios generally peaked below 10%, indicating high cell viability. After filtering, the final processed Seurat object contained 154,343 cells and 24,386 genes, representing a 98.6% data retention.
...
## Feature map plots
Feature map plots validated cluster identity and localization for clusters 8 and 3. Dlg2 was strictly localized to the Dlg2+ cluster, while S100a5 and Pcp4l1, which are activity-dependent genes stimulated by odor, showed broader distribution in the OSN clusters. Immune clusters had Cd209a and Ifi205 specific to IFN-Stim DC cluster. Ms4a4c was instead shared between stimulated DCs and non-classical monocytes. Tnfsf9 was found by Kazer et al. only in DC clusters, and it was localized at a very low expression in the IFN-Stim DC cluster, suggesting it is a baseline DC marker but not stimulated by interferons.

   ![](./plots/FeaturePlot_Cluster8.png) 
 ![](./plots/FeaturePlot_OSNs.png) 

**Figure 3:** Feature maps displaying the localization of specific marker genes in the transcriptomic landscape. A)Markers specific to the IFN-stimulated dendritic cell cluster cluster 3). B) Neuronal markers specific to olfactory sensory neurons (OSN). Colour indicates relative expression as per normalized log-counts, with blue indicating higher expression. 

## DE 
DE analysis of cluster 8 (IFN-stim DC) in the RM showed a transcriptomic shift from acute defense at 5 dpi to recovery at 14 dpi, when compared with Naïve cells. At 5 dpi (D05), a strong shift from the naïve state is shown by the robust and uniform induction of ISGs including Isg15, Rsad2, and Cxcl10. These mark the ‘defensive’ antiviral state of the cell. Biological variation was seen among replicates (column 1), though the direction of change was consistent across replicates. At 41 dpi, the transcriptome did not fully return to baseline levels but showed a less defensive profile as the ISGs were not induced. Instead, ribosomal (Rps27rt) and mitochondrial genes were activated (mt-Atp8). There is one mouse replicate (column 1) that shows a different pattern to other D14 mice. It is far from the intermediate expression seen in both baseline (Naïve) and other D14 replicates, suggesting it is taking slower to reach the rest and recovery stage than the other mice.
...
GSEA of cluster 8 confirmed the biological transitions seen in the DE analysis. At 5 dpi, the upregulated pathways surrounded viral defense response and interferon-beta signaling. At 14 dpi, there was a shift toward B cell differentiation and mRNA stability regulation, and a small downregulation of sensory perception pathways. This function shift highlights a transient shift in the transcriptome between acute antiviral states and the recovery response.
